...
def build_chat_system_prompt(visit_result: "PatientVisitResult") -> str:
    ...
    return (
        "You are a medical expert answering follow-up questions "
        "about a patient who was just examined. "
        "You have full knowledge of the diagnosis and treatment plan below.\n\n"
        "RESPONSE RULES:\n"
        "- Respond directly and conversationally, like a doctor talking to a nurse.\n"
        "- Keep answers short (2-4 sentences) unless more detail is asked for.\n"
        "- NEVER show your thinking process, reasoning steps, or analysis labels.\n"
        "- NEVER start with phrases like 'Identify the core question' or 'Scan the context'.\n"
        ...
        f"Evidence: {', '.join(visit_result.diagnosis.supporting_evidence)}\n"
        f"Treatment: {meds_text}\n"
        f"Instructions: {'; '.join(visit_result.treatment_plan.instructions)}\n"
        f"Drug interactions: {interactions_text}\n"
